...
def main():
  ...
  parser.add_argument('--type', type=str, choices=['gameplay', 'greedy'])
  ...
  #   reset_params = deepcopy(model.state_dict())
  #   X_train, y_train = dataset_test_train.X, dataset_test_train.y
  #   for t in range(args.trim_times):
  #     print('=====Round {}====='.format(t + 1))
  #     model.load_state_dict(reset_params)
  #     trainset = MyDataset(X_train, y_train)
  #     trainloader = DataLoader(trainset, batch_size=args.batch_size, shuffle=True, 
  #                             num_workers=args.num_workers, pin_memory=args.pin_memory)
  #     testloader = DataLoader(trainset, batch_size=args.batch_size, shuffle=False,
  #                             num_workers=args.num_workers, pin_memory=args.pin_memory)
  ...
  #       wandb.log({'outlier_run_train_acc': a, 'outlier_run_train_loss': b})
  #     a, b, c, d = timed_run(test, model, testloader, criterion, device, label_id)
  #     print(d.shape, X_train.shape)
  #     wandb.log({'train_size': X_train.shape[0]})
  #     # trim
  #     frac = args.outlier_frac / args.trim_times
  #     final_loss = d
  #     print(final_loss.shape)
  #     idx = np.argsort(final_loss)[:-int(frac * final_loss.shape[0])]
  ...
  if args.load_warmup is None:
    for epoch in range(args.warmup):
      ...
      if args.chi2:
        timed_run(chisq, model, trainloader, optimizer,
                criterion, None, device, 0)
      else:
        timed_run(erm, model, trainloader, optimizer,
                  criterion, None, device, 0)
    ...
    # with open("results/warmup_{}_ep{}_{}.pt".format(args.dataset, args.warmup, a), 'wb') as f:
  ...
  for epoch in range(start_epoch, args.epochs):
    ...
    sample_weights = get_sample_weights(args, sample_accuracy_history,
                                        ...
                                        obj_value, np.array(val_correct), hypothesis_weights)
    ...
    trainloader = DataLoader(dataset_train, batch_size=args.batch_size,
                             ...
                             pin_memory=args.pin_memory)
    if args.chi2:
      timed_run(chisq, model, trainloader, optimizer, criterion,
    ...
    if args.alg == "raigame":
      if args.gen_adaboost:
        ...
        hypothesis_weights = get_hypothesis_weights(args, hypothesis_weights, sample_accuracy_history)
      _, gamevalue = find_opt_w(np.array(val_correct), hypothesis_weights / hypothesis_weights.sum(), 0, args.constraints_w, args.alpha, args.verbose)
...
def get_sample_weights(args, sample_accuracy_history,
                       sample_losses_history, sample_weights_history,
                       obj_value, val_history=None, hypothesis_weights=None):
  ...
  elif args.alg == 'adaboost':
    return adaboost(sample_accuracy_history, sample_weights_history)
  elif args.alg == 'lpboost':
    return lpboost(sample_accuracy_history, obj_value, args.alpha, args.beta, args.verbose)
  elif args.alg == 'raigame':
    return raigame(sample_accuracy_history, obj_value, args.eta_rai, args.constraints_w, hypothesis_weights, args.type, args.alpha, args.verbose)
  else:
